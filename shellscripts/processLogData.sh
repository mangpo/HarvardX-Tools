# Process all of the log data from an edX dump. This script will result in a WeeklyLog for
# each course.

# The script takes 3 parameters:
# argument 1: class name in school-class-term format. E.g. BerkeleyX-CS191x-Spring_2013.
# argument 2: the day from which the log entries are to be processed 
#             (the first day of the week) in YYYY-MM-DD format.
# argument 3: the last day from which the log entries are to be processed 
#             (the last day the log files were collected) in YYYY-MM-DD format.

# All these 3 arguments can be found in info.csv generated by generateCourseAxis script

# Separate out the log entries in each directory by the class.
for line in `ls | grep prod`;
do
    cd $line
    separateClassLogs.py $1 $2 $3
    cd ..
done

# Build a log for the week for each of the classes, writing the log to the current directory
buildWeekLog.py $1





